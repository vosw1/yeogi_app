  ...
  void _handleMenuAction(BuildContext context, MenuType menuType) {
    switch (menuType) {
      ...
      case MenuType.CallCustomerService:
        // 고객 행복센터 전화 액션 추가
        _makePhoneCall(
            '010-1234-5678'); // Replace '010-1234-5678' with the actual phone number
        break;
    }
  }
...
class FAQPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FAQList(), // 자주 묻는 질문 리스트를 표시하는 위젯을 불러옵니다.
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return ListTile(
          title: Text(faqItemsData[index].question), // 질문을 리스트 타일에 표시
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) =>
                    FAQAnswerPage(faqItemsData[index]), // 탭한 아이템에 대한 답변 페이지로 이동
              ),
            );
          },
        );
